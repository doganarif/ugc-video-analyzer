...
def process_video(video_path, frames_per_second=DEFAULT_FRAMES_PER_SECOND, resize=RESIZE_OF_FRAMES, output_dir='',
    ...
    Args:
        ...
        resize (int): Factor to resize frames by (0 for no resize)
        output_dir (str): Directory to save extracted frames (if not empty)
        max_frames (int): Maximum number of frames to extract (0 for unlimited)
        
    Returns:
        list: List of base64-encoded frames
    """
    logger.info(f"Starting video processing for {video_path} with frames_per_second={frames_per_second}, resize={resize}, max_frames={max_frames}")
    base64Frames = []
    ...
    try:
        ...
        while curr_frame < total_frames - 1:
            ...
            logger.debug(f"Processing frame {curr_frame}/{total_frames} ({frame_count}/{max_frames if max_frames > 0 else 'unlimited'})")
        ...
        video.release()
        logger.info(f"Extracted {len(base64Frames)} frames from {video_path}")
...
def split_video(video_path, output_dir, shot_interval, max_duration=None):
    ...
    Args:
        video_path (str): Path to the video file
        output_dir (str): Directory to save the split shots
        shot_interval (int): Duration of each shot in seconds
        max_duration (int, optional): Maximum duration to process in seconds

    Returns:
        generator: Generator yielding (output_file_path, timeframe_string) tuples
    """
    logger.info(f"Starting video splitting for {video_path} with shot_interval={shot_interval}, max_duration={max_duration}")
    ...
    try:
        ...
        for start_time in range(0, int(duration), shot_interval):
            ...
            logger.info(f"Extracting shot from {start_time} to {end_time} into {output_file}")
            ...
            try:
                ...
                logger.error(f"Error extracting shot {start_time}-{end_time}: {ex}")
                # Continue with the next shot
                
    except Exception as ex:
        logger.error(f"Error splitting video: {ex}")
        raise 